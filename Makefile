...
# define created libs object files
CREATEDLIBS = \
    libs/1dgameplay.o \
    libs/ceasings.o \

# typing 'make' will invoke the default target entry called 'all',
# in this case, the 'default' target entry is main
all: main

# compile game -> main
main: main.c $(SCREENS) $(CREATEDLIBS)
	$(CC) -o $@$(EXT) $< $(SCREENS) $(CREATEDLIBS) $(CFLAGS) $(INCLUDES) $(LFLAGS) $(LIBS) -D$(PLATFORM) $(WINFLAGS)
...
# compile lib 1DGAMEPLAY
libs/1dgameplay.o: libs/1dgameplay.c
    $(CC) -c $< -o $@ $(CFLAGS) $(INCLUDES) -D$(PLATFORM)
    
libs/ceasings.o: libs/ceasings.c
    $(CC) -c $< -o $@ $(CFLAGS) $(INCLUDES) -D$(PLATFORM)
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    ...
    ifeq ($(PLATFORM_OS),LINUX)
		...
		del screens\*.o *.exe del libs\*.o *.exe
    endif
    endif
endif
ifeq ($(PLATFORM),PLATFORM_RPI)
...
ifeq ($(PLATFORM),PLATFORM_WEB)
	del screens/*.o *.html *.js libs\*.o *.html *.js
endif
	@echo Cleaning done
